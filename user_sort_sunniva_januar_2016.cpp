...
private:
    ...
    Histogram1Dp h_na_n, h_thick, h_ede, h_ede_r[8], h_ex, h_ex_r[8], h_ex_fiss, h_particlecount;
    
#if defined(MAKE_CACTUS_TIME_ENERGY_PLOTS) && (MAKE_CACTUS_TIME_ENERGY_PLOTS>0)
    Histogram2Dp m_nai_e_t[28], m_nai_e_t_all, m_nai_e_t_c, m_nai_e_t_ppacgated, m_siri_e_t[8], m_siri_e_t_all, m_siri_e_t_ppac[8], m_siri_e_t_all_ppac;
...
void UserXY::CreateSpectra()
{
    ...
    m_alfna_fiss = Mat( "m_alfna_fiss", "E(NaI) : E_{x} in coincidence with fission",
                  2000, -2000, 14000, "E(NaI) [keV]", 2000, -2000, 14000, "E_{x} [keV]" );  //må være likt de andre rangene for å få "multiplicieties.cpp" til å funke
    ...
    m_nai_e_t_c   = Mat( "m_nai_e_t_c", "t : E NaI all together, corrected",
                        500, 0, max_enai, "E(NaI) [keV]", 500, 0, 500, "t(NaI) [a.u.]" );
    m_nai_e_t_ppacgated   = Mat( "m_nai_e_t_ppacgated", "t : E NaI all together, corrected, gated on fission peak in PPAC",
                        500, 0, max_enai, "E(NaI) [keV]", 500, 0, 500, "t(NaI) [a.u.]" );
    
    for(int n=0; n<8; ++n ) {
        m_siri_e_t[n] = Mat( ioprintf("m_siri_e_t_b%d", n), ioprintf("t(NaI) : E(Si) detector %d", n),
                            500, 0, max_e, "E(Si) [keV]", 500, 0, 500, "t(NaI) corr. [a.u.]" );
        m_siri_e_t_ppac[n] = Mat( ioprintf("m_siri_e_t_ppac_b%d", n), ioprintf("t(PPAC) : E(Si) detector %d", n),
                            500, 0, max_e, "E(Si) [keV]", 500, 0, 500, "t(PPAC) corr. [a.u.]" );
    }
    m_siri_e_t_all = Mat( "m_siri_e_t", "t(NaI) : E(Si) all detectors",
                         500, 0, max_e, "E(Si) [keV]", 500, 0, 500, "t(NaI) corr. [a.u.]" );
    m_siri_e_t_all_ppac = Mat( "m_siri_e_t_ppac", "t(PPAC) : E(Si) all detectors",
                         500, 0, max_e, "E(Si) [keV]", 500, 0, 500, "t(PPAC) corr. [a.u.]" );
#endif /* MAKE_CACTUS_TIME_ENERGY_PLOTS */
...
//****************************************************************************************************
    ...
    for( int j=0; j<event.n_na; j++ ) {
        ...
        const int   ppac_t2 = (int)tPpac(na_t_f,e);

//        if ( na_t_f>190 && na_t_f<220 && na_e_f>1195 && na_e_f<1225 ) fis = 1;
//OBS edited fission gates for 233U data
        //if ( ppac_t2>190 && ppac_t2<204 && na_e_f>1570 && na_e_f<1700 ) fis = 1;
        //if ( ppac_t2>188 && ppac_t2<207 && e>5000) fis = 1;
        if ( ppac_t2>185 && ppac_t2<205 && e>5000) fis = 1;
    }
//****************************************************************************************************
...
//****************************************************************************************************
    ...
    if ( fis==1 )h_ex_fiss->Fill( ex_int ); //particle spectrum in coinicidence with fission
    h_ex->Fill(ex_int); //particle spectrum - everything
    ...
    for( int i=0; i<event.n_na; i++ ) {
        ...
        const int   ppac_t = (int)tPpac(na_t,e);
...
//new fission gated for 233U data
                    ...
                    //Checking that the time is for the prompt fission peak in the PPAC time spectrum
                    // and that the energy of the particle is higher than 
                    //Equivalent to selecting the fission-blob in 2D ...
            const int   ppac_tt = (int)tPpac(na_t,e);
                    //if ( ppac_tt>188 && ppac_tt<207 && e>5000 ) fiss = 1; //<-- ny gate 130515
                    //if ( ppac_tt>130 && ppac_tt<144 && e>5000  ) fiss = 2; //<-- test 18016. Denne brukes for å finne random coincidence gammas. Skal trekkes fra fisjons-alfna.
...
#if defined(MAKE_CACTUS_TIME_ENERGY_PLOTS) && (MAKE_CACTUS_TIME_ENERGY_PLOTS>0)
        if(id<28 && fis==1) m_nai_e_t[id] ->Fill( na_e_int,  na_t_int );
        if(id<28 && fis==1) m_nai_e_t_all ->Fill( na_e_int,  na_t_int );
        if(id<28 && fis==1) m_nai_e_t_c   ->Fill( na_e_int,  na_t_c );
        if(id<28 && fis==1) m_siri_e_t[ei]->Fill( e_int, na_t_c );
        if(id<28 && fis==1) m_siri_e_t_all->Fill( e_int, na_t_c );

        //Energy vs time plot gating on the fission-blob
        if(id<28 && fis==1) m_nai_e_t_ppacgated->Fill( na_e_int,  na_t_c );

        //Plots for the time-walk of the PPAC. The corrected time is found by using the function tPpac(t,e) above
        //and the result is stored in the float ppac_t
        //Now making plots for the corrected time vs energy of SiRi:

        if(id>27&&id<32) m_siri_e_t_ppac[ei]->Fill(e_int,ppac_t);
        if(id>27&&id<32) m_siri_e_t_all_ppac->Fill(e_int,ppac_t);
...
//***************************************************************************************************
        ...
        //if( id<28 && fiss==2 && na_t_c>195 && na_t_c<205 ) { //lager samme gate som for vanlig alfna
        //if( id<28 && fiss==1 && na_t_c>195 && na_t_c<205 ) { 
        //if( id<28 && fis==1 && na_t_c>180 && na_t_c<204 ) {  //gate for fission time: narrow
        if( id<28 && fis==1 && na_t_c>170 && na_t_c<220 ) {  //gate for fission time: wide
            m_alfna_fiss->Fill( na_e_int, ex_int, 1);
            //m_alfna_fiss_total->Fill( na_e_int, ex_int, 1);
        //} else if( id<28 && fiss==2 && na_t_c>102 && na_t_c<112 ) { //samme gate som for vanlig alfna_bg
        } else if( id<28 && fis==1 && na_t_c>87 && na_t_c<137 ) {   //gate for fission background, from other pulses: wide
          //m_alfna_fiss->Fill( na_e_int, ex_int, -1);
