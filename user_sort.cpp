...
private:
    ...
    Histogram2Dp m_nai_t, m_nai_e, m_nai_e_gated;
=======
     bool IsPPACChannel (int);
     int GetPPACChannel (int);
...
void UserXY::CreateSpectra()
{
    ...
    m_nai_e_gated = Mat( "m_nai_e_gated", "E(NaI) matrix (Gated 3-)", 2000, 0, 15000, "E(NaI) [keV]", 32,0,32, "det. id.");
...
>>>>>>> upstream/master
    ...
    for( int i=0; i<event.n_na; i++ ) {
        ...
        if (ex_int>6800&&ex_int<7000.) {
            m_nai_e_gated->Fill( na_e_int, id );}
